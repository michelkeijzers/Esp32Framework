...
**REQ-GPIO-100:** The generic I/O node shall support direct GPIO button inputs for simple single-button use cases (e.g. foot switches) without requiring an expander IC.<br/>
...
**REQ-GPIO-110:** The generic I/O node shall support shift register button input via 74HC165 for larger button counts.<br/>
...
**REQ-GPIO-300:** The input node shall support potentiometer input via ADS1115 ADC expander.<br/>
...
**REQ-GPIO-310:** The input node shall support Hall effect pedal input via A1324 sensor read through ADS1115.<br/>
...
**REQ-GPIO-400:** The generic I/O node shall support direct GPIO outputs for simple use cases (e.g. non dimmable LEDs) without requiring an expander IC.<br/>
...
**REQ-GPIO-410:** The generic I/O node shall support shift register output via 74HC595 for larger output counts.<br/>
...
**REQ-GPIO-500:** The input node shall send events to master on change — not on polling interval. Also, values are only sent when changing a minimum value (see REQ-GPIO-320).<br/>
...
**REQ-GPIO-520:** For outputs, a message shall be defined to set a specified output of a node to a specified value — e.g. set output 3 to 255.<br/>
**Rationale:** Provides a clear and flexible way to control outputs from the master or webserver.<br/>
