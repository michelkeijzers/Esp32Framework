...
| #3  | Input node    | Single footswitch — direct GPIO      | PCB     |
| #4  | Display node  | TM1637 seven segment, max brightness | PCB     |
| #5  | DMX node      | MAX3485, XLR, named universes in NVS | PCB     |
...
| DMX node ESP32-C3 (ESP-NOW)      | ~80mA      |
...
| ------------------ | ----------------- | -------------- | ------------------ |
| Load Configuration | Webserver         | All nodes      | -                  |
| Button Pressed     | General I/O Node  | Webserver      | Short/Long Press   |
| Select Preset      | Webserver         | DMX node       | Preset number      |
| Heartbeat          | All nodes         | Master         | Time stamp         |
| OTA Update         | Webserver         | Specific node  | TODO               |
...
| ----------- | ----------------- | ------------------------------- | ------- | ------------------------------------------------------------- |
| Master      | Mac Addresses     | mac_address\_\<x\>              | string  | MAC address of node \<x\>                                    |
| Web Server  | DMX Preset        | index                           | int16_t | Currently selected DMX preset number                          |
| DMX Node    | DMX Presets       | preset\_\<x\>\_name             | string  | Name of preset \<x\>                                          |
| DMX Node    | DMX Presets       | preset\_\<x\>\_universe         | int8_t  | Universe Number (\*2)                                         |
| DMX Node    | DMX Presets       | preset\_\<x\>\_data             | blob    | DMX values for preset \<x\>                                   |
| DMX Node    | DMX Configuration | preset\_\<x\>\_active           | int8_t  | Inactive (0), Active (1)                                      |
...
- `nodes/dmx/` — DMX output, named universe NVS storage
- `nodes/display/tm1637/` — seven segment display, brightness via software
- `nodes/input/gpio/` — direct GPIO button / footswitch

**Build order:**

1. Master — verify boot, ESP-NOW init, MAC on serial
2. Input node — verify heartbeat and button press in master serial
3. Webservers nodes — verify status page on phone, nodes shown online
4. Display node — send test message from webserver nodes, verify on display
5. DMX node— verify DMX output, universe switching, NVS storage
