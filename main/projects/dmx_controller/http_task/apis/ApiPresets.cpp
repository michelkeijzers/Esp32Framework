...
#include "../../../../esp/esp_http_server/UriParamExtractor.hpp"
#include <sstream>
...
esp_err_t ApiPresets::select_preset_handler(httpd_req_t *req) {
    ...
    std::string resp = "{\"preset_number\":" + std::to_string(preset->number) + ",";
    resp += "\"preset_name\":\"" + preset->name + "\"}";
...
esp_err_t ApiPresets::move_preset_up_handler(httpd_req_t *req) {
    ...
    uint32_t preset_num = UriParamExtractor::extractFirstParamAsUint32(
        "/api/v1/presets/*/move_up", req->uri);
...
esp_err_t ApiPresets::move_preset_down_handler(httpd_req_t *req) {
    ...
    uint32_t preset_num = UriParamExtractor::extractFirstParamAsUint32(
        "/api/v1/presets/*/move_down", req->uri);
...
esp_err_t ApiPresets::insert_preset_at_handler(httpd_req_t *req) {
    ...
    uint32_t position = UriParamExtractor::extractFirstParamAsUint32(
        "/api/v1/presets/*/insert_at", req->uri);
...
esp_err_t ApiPresets::swap_preset_activation_handler(httpd_req_t *req) {
    ...
    uint32_t preset_num = UriParamExtractor::extractFirstParamAsUint32(
        "/api/v1/presets/*/swap_activation", req->uri);
