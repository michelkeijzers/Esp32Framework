...
struct spi_flash_chip_t {
    ...
    esp_err_t (*write)(struct esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length);
    esp_err_t (*program_page)(struct esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length);
    uint32_t page_size;
    esp_err_t (*write_encrypted)(struct esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length);
...
struct esp_flash_t {
    ...
    const struct spi_flash_chip_t *chip_drv;   ///< Pointer to chip-model-specific "adapter" structure. If NULL, will be detected during initialisation.
